...
export async function promoteUser(ctx: CallbackQueryContext<Context>) {
  ...
  try {
    ...
    if (user.group_id === UserGroup.Admin) {
      ...
      await repositories.users.updateUser({ id: userId, name: user.name, group_id: UserGroup.Admin, user_folder: user.user_folder });
    ...
    logger.error(`Error in admin.service > userPromote: ${error instanceof Error ? error.message : error}`);
    return false;
  }
};
