...
export async function requestNewUserName(ctx: CallbackQueryContext<Context>) {
  ...
  try {
    ...
    session.scene = 'enter_name';
    session.customData = session.customData || {};
    session.customData.userId = userId;

    logger.info(`User ID ${userId} stored in session for renaming.`);
  } catch (error: unknown) {
    if (error instanceof Error) {
      logger.error(`Error in admin.service > requestNewUserName: ${error.message}`);
    } else {
      logger.error('An unknown error occurred in admin.service > requestNewUserName.');
    }
  }
};
