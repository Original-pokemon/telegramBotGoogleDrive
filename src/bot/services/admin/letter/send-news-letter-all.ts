  ...
  try {
    ...
    const promises = users.map(async ({ id }) => {
      try {
        ...
        ctx.logger.error(`Failed to send message to user with id ${id}: ${error instanceof Error ? error.message : error}`);
      }
    });
    ...
    ctx.logger.error(`Error in admin.service > sendNewsletterForAll: ${error instanceof Error ? error.message : error}`);
  }
};