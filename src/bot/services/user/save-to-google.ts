...
export async function saveToGoogle(ctx: Context) {
  ...
  try {
    let folderId = await ctx.repositories.photoFolders.getUserFolderByDate(userId, date);
    ...
    await sendPhotosToDrive(repositories.googleDrive, _.compact(answers), folderId);
