...
import { sendEndMessage } from "./index.js";
import { sendQuestionMessage } from "./index.js";

async function sendNextMessage(ctx: Context, answersLength: number, questionsLength: number) {
  const { logger } = ctx;

  try {
    if (answersLength > questionsLength) {
      logger.warn(`Answers length (${answersLength}) exceeds questions length (${questionsLength}).`);
      return;
    }
    ...
    if (questionsLength === answersLength) {
      ...
      logger.debug(`Sending next question. Answers so far: ${answersLength}/${questionsLength}.`);
